...
for d in ['test','train']:
            ...
            for y in range(stride):
                for x in range(stride):
                    ...
                    if tnum==1:
                        ...
                        HSVBlue = np.zeros((thisSlice.shape[0],thisSlice.shape[1]))
                        HSVBlue=np.abs(this_hsv_Slice[:,:,0]-120)
                        HSVBlue[HSVBlue>15]=0.
                        HSVBlue[HSVBlue>0]=1.
                        results[y*stride+x,5]=bigBlue.sum()/HSVBlue.size
    ...
    varnames.append("veryHSVBlueProp")
    df.columns = varnames
    df.index = df['img_num']
